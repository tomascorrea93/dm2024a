...
Estrategia_C <- function() {
  ...
  for (ronda in 2:4) {
    ...
    tiros_ronda <- c(70, 70, 70)[ronda - 1] 
    resultados2 <- gimnasio_tirar(planilla_cazatalentos$ids_juegan1, tiros_ronda)
     # Acumular aciertos
  }
  planilla_cazatalentos[, aciertos := aciertos + round(median(resultados2))]
  
  #Ronda 5 - seleccion
  planilla_cazatalentos <- planilla_cazatalentos[order(-aciertos)[1:5]]
  resultados3 <- gimnasio_tirar(planilla_cazatalentos$ids_juegan1, 90)
  planilla_cazatalentos[, aciertos := aciertos + resultados3]
